...
{/*{{{*/
    ...
    switch(priority) {
    case LOG_DEBUG:
        fprintf(fp_log, "[%s][%s][FILE:%s LINE:%d][threadid: %lu] %s\n", sz_date, "DEBUG", filename, line, pthread_num, sz_buf);
        fflush(fp_log);
        break;  
    case LOG_ERROR:
        fprintf(fp_log, "[%s][%s][FILE:%s LINE:%d][threadid: %lu] %s\n", sz_date, "ERROR", filename, line, pthread_num, sz_buf);
        fflush(fp_log);
        break;
    case LOG_INFO:
        fprintf(fp_log, "[%s][%s][FILE:%s LINE:%d][threadid: %lu] %s\n", sz_date, "INFO", filename, line,  pthread_num, sz_buf);
        fflush(fp_log);
        break;
    case LOG_SQL:
        fprintf(fp_log, "[%s][%s][FILE:%s LINE:%d][threadid: %lu] %s\n", sz_date, "EXEC SQL", filename, line, pthread_num, sz_buf);
        fflush(fp_log);
        break;
    case LOG_STAT:
        fprintf(fp_log, "[%s][%s][FILE:%s LINE:%d][threadid: %lu] %s\n",sz_date, "DUMP", filename, line, pthread_num, sz_buf);
        fflush(fp_log);
        break;
    default :
        fprintf(fp_log, "[%s][FILE:%s LINE:%d][threadid: %lu] %s\n", sz_date, filename, line, pthread_num, sz_buf);
        fflush(fp_log);
        break;
    }
#ifndef DEBUG
...
/// @returns 是否正确初始化
// --------------------------------------------------------------------------
int32_t init_log(char *_log_file_dir, char *_log_file_name, char *_program_name)
{/*{{{*/
